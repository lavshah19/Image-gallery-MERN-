...
/*
The Workflow Recap:
Multer stores the file temporarily in the uploads/ folder.

The image is uploaded to Cloudinary for permanent storage.

The local copy is deleted after a successful upload to keep your server clean.

Cloudinary URL and other details (like publicId) are saved to the database. 




In Summary:
Multer helps to receive, validate, and store the file temporarily while it's being uploaded.

Cloudinary is the final storage for the file.

Local deletion is performed to free up space on your server after the file is safely stored in the cloud.

If you don't need to store the file locally at all, you could upload directly to Cloudinary from the client side (using their SDKs), but it limits the control you have over the file before uploading.
*/